...
@section('title', 'Edit Chart of Account: ' . $chartOfAccount->account_name)

@section('content')
<div class="max-w-3xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Edit Chart of Account</h1>
        <p class="text-gray-600 mt-1">Update account: {{ $chartOfAccount->account_name }}</p>
    </div>

    <form action="{{ route('chart-of-accounts.update', $chartOfAccount) }}" method="POST" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
                ...
                >
                    ...
                    <option value="current_asset" {{ old('account_category', $chartOfAccount->account_category) == 'current_asset' ? 'selected' : '' }}>Current Asset</option>
                    <option value="fixed_asset" {{ old('account_category', $chartOfAccount->account_category) == 'fixed_asset' ? 'selected' : '' }}>Fixed Asset</option>
                    <option value="current_liability" {{ old('account_category', $chartOfAccount->account_category) == 'current_liability' ? 'selected' : '' }}>Current Liability</option>
                    <option value="long_term_liability" {{ old('account_category', $chartOfAccount->account_category) == 'long_term_liability' ? 'selected' : '' }}>Long Term Liability</option>
                    <option value="equity" {{ old('account_category', $chartOfAccount->account_category) == 'equity' ? 'selected' : '' }}>Equity</option>
                    <option value="revenue" {{ old('account_category', $chartOfAccount->account_category) == 'revenue' ? 'selected' : '' }}>Revenue</option>
                    <option value="expense" {{ old('account_category', $chartOfAccount->account_category) == 'expense' ? 'selected' : '' }}>Expense</option>
                    <option value="cost_of_sales" {{ old('account_category', $chartOfAccount->account_category) == 'cost_of_sales' ? 'selected' : '' }}>Cost of Sales</option>
                </select>
            ...
            <div>
                ...
                <input 
                    ...
                    id="opening_balance" 
                    value="{{ old('opening_balance', $chartOfAccount->opening_balance ?? 0) }}" 
                    step="0.01"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#009245] focus:border-transparent"
                    placeholder="0.00"
                >
            ...
            <div>
                ...
                <input 
                    ...
                    name="sort_order" 
                    id="sort_order" 
                    value="{{ old('sort_order', $chartOfAccount->sort_order ?? 0) }}" 
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#009245] focus:border-transparent"
                    placeholder="0"
                >
            ...
            <div>
                ...
                <div class="flex items-center space-x-3 mt-2">
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $chartOfAccount->is_active) ? 'checked' : '' }} class="w-4 h-4 text-[#009245] border-gray-300 rounded focus:ring-[#009245]">
            ...
            <div class="sm:col-span-2">
                ...
                >{{ old('description', $chartOfAccount->description) }}</textarea>
                @error('description')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>
        </div>

        <div class="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200">
            <a href="{{ route('chart-of-accounts.show', $chartOfAccount) }}" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 font-semibold">
                Cancel
            </a>
